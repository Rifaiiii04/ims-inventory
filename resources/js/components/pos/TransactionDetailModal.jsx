...
function TransactionDetailModal({ transaction, onClose }) {
    ...
    const handleWhatsApp = () => {
        const message = `Struk Transaksi\n\nNo. Transaksi: TRX${transaction.id_transaksi}\nTanggal: ${formatDate(transaction.tanggal_waktu)}\nTotal: ${formatPrice(transaction.total_transaksi)}\nMetode: ${getPaymentMethodText(transaction.metode_bayar)}\n\nTerima kasih!`;
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500">Struk transaksi lengkap</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4">
                        <h4 className="font-bold text-gray-800 mb-3">Informasi Transaksi</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                <span className="text-gray-600">Nomor Transaksi:</span>
                                <span className="font-semibold ml-2">TRX{transaction.id_transaksi}</span>
                            </div>
                            <div>
                                <span className="text-gray-600">Waktu:</span>
                                <span className="font-semibold ml-2">{formatDate(transaction.tanggal_waktu)}</span>
                            </div>
                            <div>
                                <span className="text-gray-600">Metode Pembayaran:</span>
                                <span className="font-semibold ml-2">{getPaymentMethodText(transaction.metode_bayar)}</span>
                            </div>
                            <div>
                                <span className="text-gray-600">Kasir:</span>
                                <span className="font-semibold ml-2">{transaction.user?.nama_user || 'Unknown'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Items List */}
                    <div>
                        <h4 className="font-bold text-gray-800 mb-3">Daftar Item</h4>
                        <div className="space-y-2">
                            {transaction.details?.map((detail, index) => (
                                <div key={index} className="flex justify-between items-center py-2 border-b border-gray-100">
                                    <div className="flex-1">
                                        <p className="font-medium text-gray-800 text-sm">
                                            {detail.variant?.nama_varian || 'Unknown Item'}
                                        </p>
                                        <p className="text-xs text-gray-500">
                                            {formatPrice(detail.harga_satuan)} x {detail.jumlah}
                                        </p>
                                    </div>
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                        ...
                        {transaction.catatan && (
                            <div className="mt-2">
                                ...
                                <p className="text-sm text-gray-700 mt-1">{transaction.catatan}</p>
                            </div>
                        )}
                    </div>
