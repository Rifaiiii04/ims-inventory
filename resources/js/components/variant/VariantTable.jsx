...
function VariantTable({ data, onEdit, onDelete }) {
    ...
    const getVariantColor = (name) => {
        const colors = {
            Bakar: "from-orange-100 to-red-200 text-orange-700",
            Goreng: "from-yellow-100 to-orange-200 text-yellow-700",
            Manis: "from-pink-100 to-purple-200 text-pink-700",
            Tawar: "from-blue-100 to-cyan-200 text-blue-700",
            Spesial: "from-purple-100 to-pink-200 text-purple-700",
            Pedas: "from-red-100 to-orange-200 text-red-700",
        };
    ...
    return (
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden backdrop-blur-sm">
            <div className="bg-gradient-to-r from-indigo-50 via-white to-cyan-50 px-6 py-5 border-b border-gray-200/50">
                        ...
                        <div className="w-10 h-10 bg-gradient-to-br from-indigo-500 to-cyan-500 rounded-xl flex items-center justify-center shadow-lg">
                            ...
                            >
                                <path
                                    ...
                                    d="M7.5 8.25h9m-9 3H12m-9.75 1.5c0 1.242 1.007 2.25 2.25 2.25h13.5c1.243 0 2.25-1.008 2.25-2.25v-9a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 5.25v9a2.25 2.25 0 002.25 2.25z"
                                />
                            </svg>
                        </div>
                        <div>
                ...
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200/50">
                        {data.map((item, index) => (
                            <tr
                                key={item.id}
                                className={`group hover:bg-gradient-to-r hover:from-gray-50 hover:to-indigo-50/30 transition-all duration-200 ${
                                    index % 2 === 0
                                        ? "bg-white"
                                        : "bg-gray-50/30"
                                }`}
                            >
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className={`w-8 h-8 bg-gradient-to-br ${getVariantColor(
                                                item.name
                                            )} rounded-lg flex items-center justify-center font-bold text-sm`}
                                        >
                                            {getVariantIcon(item.name)}
                                        </div>
                                        <div>
            ...
            <div className="md:hidden divide-y divide-gray-200/50">
                {data.map((item, index) => (
                    <div
                        key={item.id}
                        className={`p-5 hover:bg-gradient-to-r hover:from-gray-50 hover:to-indigo-50/30 transition-all duration-200 ${
                    ...
                    >
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                <div
                                    className={`w-10 h-10 bg-gradient-to-br ${getVariantColor(
                                        item.name
            ...
            {data.length === 0 && (
                <div className="text-center py-16 px-6">
                    ...
                    <p className="text-gray-500 text-sm mb-6 max-w-sm mx-auto">
                        Mulai kelola varian produk Anda dengan menambahkan
                        varian pertama
                    </p>
                    <div className="flex items-center justify-center gap-2 text-xs text-gray-400">
                        <div className="w-1 h-1 bg-gray-300 rounded-full"></div>
                        <span>Klik tombol "Tambah Varian" untuk memulai</span>
                        <div className="w-1 h-1 bg-gray-300 rounded-full"></div>
                    </div>
                </div>
            )}
