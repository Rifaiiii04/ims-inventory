...
function NotificationManagement() {
    ...
    const handleBulkDelete = async () => {
        if (selectedPredictions.length === 0) {
            showToast(
                "Pilih prediksi yang akan dihapus terlebih dahulu",
                "warning"
            );
            return;
        }
        ...
        try {
            ...
            showToast(
                "Terjadi kesalahan saat menghapus prediksi expired",
                "error"
            );
        } finally {
            setDeletingPredictions(false);
    ...
    const handleGenerateExpiredPredictions = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                let message = `Prediksi expired berhasil di-generate! Berhasil: ${data.berhasil}, Gagal: ${data.gagal}`;
                if (data.dilewati && data.dilewati > 0) {
                    message += `. Dilewati: ${data.dilewati} (sudah memiliki prediksi)`;
                }
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                                    <div>
                                        ...
                                        <p className="text-gray-600">
                                            Notifikasi otomatis dikirim ketika
                                            stok bahan di bawah batas minimum
                                        </p>
                                        {!notificationEnabled &&
                                            !loadingSettings && (
